...
public class ProviderActivityEs extends AppCompatActivity {

    NetworkChangeListener networkChangeListener = new NetworkChangeListener();
    ImageButton btnRegresar;
    Button btnSiguiente;
    CheckBox avoiding, including, check1, check2, check5, check6;
    EditText check3, check4, ProfesionalQ, otroComentario;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_provider_es);

        SharedPreferences preferencia = getSharedPreferences("Datos",MODE_PRIVATE);
        SharedPreferences.Editor editor = preferencia.edit();

        btnSiguiente = findViewById(R.id.btnProviderSiguiente);
        btnRegresar = findViewById(R.id.btnProviderRegresar);
        ProfesionalQ = findViewById(R.id.editProfesional);
        check1 = findViewById(R.id.checkBoxProvider1);
        check2 = findViewById(R.id.checkBoxProvider2);
        check5 = findViewById(R.id.checkBoxProvider5);
        check6 = findViewById(R.id.checkBoxProvider6);
        avoiding = findViewById(R.id.checkBoxProvider3);
        including = findViewById(R.id.checkBoxProvider4);
        otroComentario = findViewById(R.id.editOtroComentario);
        check3 = findViewById(R.id.editCheck3);
        check4 = findViewById(R.id.editCheck4);

        avoiding.setOnClickListener(view -> {
            check3.setEnabled(avoiding.isChecked());
        });

        including.setOnClickListener(view -> {
            check4.setEnabled(including.isChecked());
        });
        ...
        btnSiguiente.setOnClickListener(view -> {
            if (!ProfesionalQ.getText().toString().isEmpty()){editor.putString("ProfesionalQualified",ProfesionalQ.getText().toString());}
            if(check1.isChecked()){editor.putString("Profesional1","Si");}
            if(check2.isChecked()){editor.putString("Profesional2","Si");}
            if(check5.isChecked()){editor.putString("Profesional5","Si");}
            if(check6.isChecked()){editor.putString("Profesional6","Si");}
            if(avoiding.isChecked()){
                editor.putString("ProfesionalAvoiding",check3.getText().toString());
            }
            if(including.isChecked()){
                editor.putString("ProfesionalIncluding",check4.getText().toString());
            }
            if(!otroComentario.getText().toString().isEmpty()){editor.putString("ProfesionalComentario", otroComentario.getText().toString());}
            editor.putString("Provedor","si");
            editor.apply();
    ...
    private void registerPerson() {
        ...
        AsyncTask.execute(() -> {
            try {
                ...
                if (code == 200) {
                    Intent intent = new Intent(this, Recomendacion1ActivityEs.class);
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    private void enviarEmail(){
        ...
        javaMailAPI.execute();
        JavaMailAPI javaMailAPITrab = new JavaMailAPI(this,preferencia.getString("EmailTr","N/A"),"Formulario "+preferencia.getString("Nombre","N/A"),"Formulario",getFilePath(),preferencia.getString("Nombre","N/A")+".pdf");
        javaMailAPITrab.execute();
    }
    ...
    public void crearPDF(String path) {
        try {
            ...
            Paragraph p3 = new Paragraph(
                    ...
                    FontFactory.getFont("arial", 20, Font.BOLDITALIC, BaseColor.BLACK)
            );
